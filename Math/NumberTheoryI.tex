...
如果假定除法是$O(1)$的話，由$\gcd(a,b)=\gcd(b,a \mod b)$可以$O(\log C)$求得兩數的最大公因數。不失一般性假設$a>b$，因為$a \mod b < b$，所以$a \geq b+(a \mod b) > 2\cdot(a \mod b)$，也就是說每做一次有效的模，$a$的值就會砍半一次，最多砍半$O(\lg(a)+\lg(b))$次。通常可以直接用gcc的\inline{\_\_gcd}，但是負數要好好處理。
